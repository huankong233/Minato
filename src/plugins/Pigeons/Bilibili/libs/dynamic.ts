...
const additionalFormatters = {
  ...
  ADDITIONAL_TYPE_VOTE: ({ vote: { desc, end_time, join_num } }) => [
    Structs.text(
      [`【投票】${desc}`, `截止日期：${new Date(end_time * 1000).toLocaleString()}`, `参与人数：${humanNum(join_num)}`, '投票详情见原动态'].join(
        '\n',
      ),
    ),
  ],
...
const majorFormatters = {
  ...
  MAJOR_TYPE_ARCHIVE: ({ archive: { cover, aid, bvid, title, stat } }) => [
    Structs.image(cover),
    Structs.text([`av${aid}`, title?.trim(), `${stat.play}播放 ${stat.danmaku}弹幕`, `https://www.bilibili.com/video/${bvid}`].join('\n')),
  ],

  // 文章
  MAJOR_TYPE_ARTICLE: ({ article: { covers, id, title, desc } }) => [
    ...(covers.length ? [Structs.image(covers[0])] : []),
    Structs.text([`《${title?.trim()}》`, desc?.trim(), `https://www.bilibili.com/read/cv${id}`].join('\n')),
  ],

  // 音乐
  MAJOR_TYPE_MUSIC: ({ music: { cover, id, title, label } }) => [
    Structs.image(cover),
    Structs.text([`au${id}`, title?.trim(), `分类：${label}`, `https://www.bilibili.com/audio/au${id}`].join('\n')),
  ],
  ...
  MAJOR_TYPE_LIVE: ({ live: { cover, title, id, live_state, desc_first, desc_second } }) => [
    Structs.image(cover),
    Structs.text(
      [title, `房间号：${id}`, `分区：${desc_first}`, live_state ? `直播中  ${desc_second}` : '未开播', `https://live.bilibili.com/${id}`].join('\n'),
    ),
  ],
...
const formatDynamic = async (item: any) => {
  ...
  const lines = [Structs.text([`https://t.bilibili.com/${item.id_str}`, `UP: ${author.name}`].join('\n'))]
